...
def get_token(mydigilearn_token: Annotated[str | None, Cookie()] = None):
    ...
    validate_token(mydigilearn_token)
...
def login_app(
    ...
    if login_response["status_code"] != 200:
        return {
            "status_code": 400,
            "status": "fail",
            "message": "Login mydigilearn gagal, token gagal diambil",
...
def logout_app(
    ...
    response.delete_cookie(settings.MYDIGILEARN_TOKEN_NAME)
    return {
        "status_code": 200,
        "status": "success",
...
def sync_db(token: str = Depends(get_token)):
