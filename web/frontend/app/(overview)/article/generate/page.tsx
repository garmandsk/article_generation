    ...
    const fetchInitialData = async () => {
      ...
      try {
        ...
        if (result && result.status === "success" && result.data?.topics) {
          ...
          sysLog(
            "success",
            "Sukses menarik data topics dan keywords dari backend",
            result.exec_time
          );
        }
      } catch (error) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">
              Hasilkan konten artikel berkualitas berdasarkan topik dan kata kunci AI.
            </p>
          </div>
        </div>
      </div>
                  ...
                  <div className="flex items-center justify-between mb-3 border-b border-slate-800/50 pb-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        value={topicLimit}
                        onChange={(e) =>
                          setTopicLimit(e.target.value === "all" ? "all" : Number(e.target.value))
                        }
                        className="bg-[#0A0E1A] text-[10px] text-slate-400 border border-slate-700 rounded px-1.5 py-0.5 outline-none cursor-pointer hover:border-slate-500 transition-colors"
                      >
              ...
              <div className="space-y-1 mt-2 relative">
                ...
                <div className="relative">
                  ...
                  <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                    <ChevronDown
                      size={16}
                      className={`text-slate-500 transition-transform duration-200 ${isModelDropdownOpen ? "rotate-180" : ""}`}
                    />
                  </div>
                </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {formData.keywords.length > 0 ? (
                formData.keywords.map((k) => (
                  <span
                    key={`mod-k-${k}`}
                    className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2.5 py-1 rounded-full text-xs"
                  >
                    {k}
                  </span>
                ))
