  ...
  const handleLogin = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok && !(data.status_code == 200)) {
        ...
        setErrorMessage(
          pesanDariServer || "Login gagal. Periksa kembali username & password Anda."
        );
      }
  ...
  return (
    ...
    */
    <div className="w-full max-w-md bg-[#002642]/80 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-8 shadow-2xl relative z-10">
      ...
      {/* Notifikasi Error */}
      {errorMessage && (
        <div className="mb-6 p-4 bg-[#840032]/20 border border-[#840032] rounded-lg flex items-start gap-3">
        ...
        <div className="flex items-center">
          <label className="flex items-center gap-2 cursor-pointer group">
            <div className="relative flex items-center justify-center">
              ...
              />
              <svg
                className="absolute w-3 h-3 text-[#02040F] opacity-0 peer-checked:opacity-100 pointer-events-none"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={3}
